...
app.put('/tasks/:id', async (req, res) => {
  try {
    ...
    const updatedTitle = typeof title === 'string' && title.trim() ? title.trim().slice(0, 50) : existing.title;
    const updatedDescription = typeof description === 'string' ? description : existing.description;
    const updatedPriority = ['Low', 'Medium', 'High'].includes(priority) ? priority : existing.priority;
...
app.patch('/tasks/:id/status', async (req, res) => {
  try {
    ...
    const newStatus = typeof isDone === 'boolean' ? (isDone ? 1 : 0) : (existing.isDone ? 0 : 1);
    await db.run('UPDATE tasks SET isDone = ? WHERE id = ?', newStatus, id);
    const task = await db.get('SELECT * FROM tasks WHERE id = ?', id);
    res.json(task);
  } catch (err) {
    console.error(err);
