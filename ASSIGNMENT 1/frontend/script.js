...
function applyFilters(inputTasks) {
  ...
  const filtered = inputTasks.filter((task) => {
    if (currentFilter === 'active' && task.isDone) return false;
    if (currentFilter === 'completed' && !task.isDone) return false;
...
function startEditTask(task) {
  ...
  const description = prompt('Edit task description (optional)', task.description || '');
...
function initEventHandlers() {
  elements.form.addEventListener('submit', (event) => {
    ...
    const priority = normalizePriority(elements.priority.value);
    if (!title) return;
  ...
  elements.sort.addEventListener('change', () => renderTasks());
}
